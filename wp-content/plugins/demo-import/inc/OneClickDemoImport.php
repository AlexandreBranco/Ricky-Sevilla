...
class OneClickDemoImport {
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( $exc_file ) {
			$this->append_to_frontend_error_messages( $this->import_exec( $exc_file ) );
		}
	...
	public function import_exec( $import_file_path ) {
		...
		if ( ! empty( $import_file_path ) ) {
			if ( @file_exists( $import_file_path ) ) {
				...
				if ( class_exists( '\RevSliderSlider' ) && isset( $ocdi_slides_to_revolution ) && count( $ocdi_slides_to_revolution ) > 0 ) {
